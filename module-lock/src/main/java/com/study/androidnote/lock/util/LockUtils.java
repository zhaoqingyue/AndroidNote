package com.study.androidnote.lock.util;
...
public class LockUtils {
    ...
    public static int changeSize(Context context, String value) {
        if (value.contains("dip")) {
            ...
            return SizeUtils.dp2px(dip);
        } else if (value.contains("dp")) {
            float dp = Float.valueOf(value.substring(0, value.indexOf("dp")));
     ...
     * @param offset the offset to the frame of the circle (if offset > 0 : the offset is inside the circle; if offset < 0 : the offset is outside the circle)
     */
     ...
     */
    public static byte[] patternToHash(List<LockCell> pattern) {
        if (pattern == null) {
            return null;
        } else {
     ...
     */
    public static boolean checkPattern(List<LockCell> pattern, byte[] bytes) {
