...
import com.study.commonlib.util.zqy.DateUtils;
...
//    }
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        LogUtils.e("ZQY", "days: " + days.toString() + ", day: " + day);
        selectedDayIndex = findItemIndex(days, day);
        LogUtils.e("ZQY", "selectedDayIndex: " + selectedDayIndex);
...
//                        LogUtils.verbose(this, "change minutes after hour wheeled");
    ...
    protected void onSubmit() {
        if (onDateTimePickListener == null) return;
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        ...
        days.clear();
        if (selectedYear == startYear && selectedMonth == startMonth && selectedYear == endYear && selectedMonth == endMonth) {
            //开始年月及结束年月相同情况
