...
import com.study.commonlib.util.constant.RegexConstants;
...
public final class RegexUtils {
    ...
    /**
     * 验证输入信息是否合法（仅支持特定符号、汉字、数字、字母的输入）
     */
    public static boolean isNormalText(String str) {
        return isMatch(ConstUtils.REGX_TEXT_COMMON, str);
    }

    /**
     * 编辑框输入内容与后台关键字匹配的合法性判断
     */
    public static boolean isKeywordsValid(String str) {
        String[] s = ConstUtils.REGX_KEYWORDS.split("\\|");
        return !(!TextUtils.isEmpty(str) && Arrays.asList(s).contains(str));
    }
