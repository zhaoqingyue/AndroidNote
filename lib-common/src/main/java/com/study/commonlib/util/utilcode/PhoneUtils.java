...
public class PhoneUtils {
     ...
     * 将多个电话号码分开，并返回号码列表
     */
    public static ArrayList<String> getPhoneList(String dealerPhone) {
        String Servicephone;
        ArrayList<String> phone_list = new ArrayList<String>();
        if(!TextUtils.isEmpty(dealerPhone)) {
            if (dealerPhone.contains("/")) {
                String[] phones = dealerPhone.split("/");
                for (int i = 0; i < phones.length; i++) {
                    if (phones[i].contains("转")) {
                        String[] telephone = phones[i].split("转");
                        Servicephone = telephone[0];
                    } else {
                        Servicephone = phones[i];
                    }
                    phone_list.add(Servicephone);
                }

            } else {
                if (dealerPhone.contains("转")) {
                    String[] telephone = dealerPhone.split("转");
                    Servicephone = telephone[0];
                } else {
                    Servicephone = dealerPhone;
                }
                phone_list.add(Servicephone);
            }
        }
